...
RerouteResult a_star_reroute_multithread(
    ...
    // 2. 提前声明所有需要私有化的变量（必须在parallel for之前声明！）
    size_t team_idx;                                                                 // 匹配循环变量类型
    decltype(all_fail_bumps)::iterator fail_bumps_iter;                               // 显式声明迭代器类型
    const std::vector<std::pair<size_t, int>>* fail_bumps;                            // 指针私有化，避免拷贝
    const PairTeam* pt;                                                               // 指针私有化PairTeam

    // 3. OpenMP并行循环（修正private子句+无重定义）
    ...
    // 循环内不再重新定义变量，仅给提前声明的变量赋值
    for (team_idx = 0; team_idx < vpt.size(); ++team_idx) {
        fail_bumps_iter = all_fail_bumps.find(team_idx);  // 给提前声明的迭代器赋值
        if (fail_bumps_iter == all_fail_bumps.end()) {
            continue;
        }
        fail_bumps = &(fail_bumps_iter->second);          // 取地址赋值给指针，避免拷贝
        pt = &vpt[team_idx];                               // 取地址赋值给PairTeam指针

        // 直接调用函数（解引用指针，匹配原函数参数类型）
        // IOUtils::print("Rerouting Team "+std::to_string(team_idx)+"...\n");
        a_star_reroute_per_team(
            rc,
            *pt,                                           // 解引用为const PairTeam&，匹配原参数类型
            *fail_bumps,                                   // 解引用为const vector&，匹配原参数类型
            reroute_fail_nets,
            escape_results,
            fail_nets_mutex,       // 锁继续沿用，无需修改
            escape_results_mutex,   // 锁继续沿用，无需修改
            rc_mutex
        );
    }
