...
std::vector<AStarNodeDirect> get_neighbors_direct_lock(
        ...
        std::shared_mutex& rc_mutex  // 修改：mutex → shared_mutex
) {
    std::vector<AStarNodeDirect> neighbors;
    std::vector<std::tuple<int, int, int>> dirs = {
            std::make_tuple(0, 1, 0),
    ...
    for (const auto& dir : dirs) {
        ...
        if (new_x >= 0 && new_x < max_width && new_y >= 0 && new_y < max_height && new_layer >= 0 && new_layer < layer_count) {
            ...
            {
                std::shared_lock<std::shared_mutex> lock(rc_mutex);  // 修改：shared_lock替代lock_guard
...
std::vector<AStarNodeDirect> reconstruct_path_direct_lock(
    ...
    while (current != nullptr) {
        path.push_back(*current);
        // 写操作：独占锁（lock_guard兼容shared_mutex，自动独占）
        {
            std::lock_guard<std::shared_mutex> lock(rc_mutex);  // 修改：mutex → shared_mutex
...
void a_star_reroute_per_team(
        ...
        std::vector<std::string> &reroute_fail_nets,
        std::unordered_map<std::string, std::vector<std::pair<ResultPoint, ResultPoint>>> &escape_results,
        std::mutex& fail_nets_mutex,    // 这个锁是写专属，保持mutex即可
        std::mutex& escape_results_mutex,  // 这个锁是写专属，保持mutex即可
        std::shared_mutex& rc_mutex     // 修改：mutex → shared_mutex
)
{
    for (const auto& fail_pair : fail_bumps) {
        ...
        // 1. 保护reroute_fail_nets的push_back操作（写锁）
        if (routing_result.path.empty() && !(fail_bump.grid_coord_x == fail_bump.exit_terminal.grid_coord_x && fail_bump.grid_coord_y == fail_bump.exit_terminal.grid_coord_y)) {
            std::lock_guard<std::mutex> lock(fail_nets_mutex);
            reroute_fail_nets.push_back(net_name);
        }
        ...
        // 2. 保护escape_results的insert操作（写锁）
...
RerouteResult a_star_reroute_multithread(
    ...
    // 1. 移除线程池和future相关代码，新增OpenMP线程数设置
    omp_set_num_threads(8); // 对应原ThreadPool的线程数

    // 2. 提前声明所有需要私有化的变量
    size_t team_idx;
    decltype(all_fail_bumps)::iterator fail_bumps_iter;
    const std::vector<std::pair<size_t, int>>* fail_bumps;
    const PairTeam* pt;

    // 3. OpenMP并行循环
    #pragma omp parallel for \
        private(team_idx, fail_bumps_iter, fail_bumps, pt) \
        shared(vpt, all_fail_bumps, rc, reroute_fail_nets, escape_results, fail_nets_mutex, escape_results_mutex, rc_mutex)
    for (team_idx = 0; team_idx < vpt.size(); ++team_idx) {
        fail_bumps_iter = all_fail_bumps.find(team_idx);
        if (fail_bumps_iter == all_fail_bumps.end()) {
            continue;
        }
        fail_bumps = &(fail_bumps_iter->second);
        pt = &vpt[team_idx];
