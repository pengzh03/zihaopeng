...
def main(argv):
    # 支持两种参数格式：
    # 1. 无min_layers: evaluator.py grid.json netlist.json routing.json report_path
    # 2. 有min_layers: evaluator.py grid.json netlist.json routing.json report_path min_layers
    if len(argv) not in (5, 6):
        print("Usage: python evaluator.py grid_layout.json netlist.json routing.json report_output_path [min_layers]")
        sys.exit(2)
    ...
    netlist_path = argv[2]
    routing_path = argv[3]
    report_output_path = argv[4]  # 从第四个参数获取输出路径
    min_layers = None
    
    if len(argv) == 6:
        try:
            min_layers = float(argv[5])
        except Exception as e:
    ...
    # 关键修改：把评测结果直接写入指定的输出路径
    try:
        with open(report_output_path, 'w', encoding='utf-8') as f:
            json.dump(report, f, ensure_ascii=False, indent=2)
        print(f"Report written to {report_output_path}")  # 这句只是打印提示，不影响文件内容
    except Exception as e:
        print(json.dumps({'error': str(e)}))
        sys.exit(1)
